...
export class InvoiceDetails extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      has_discount:false,   //// discount false
      discount_ref : 0,  /// previous apply discount
      discount: 0,  /// current discount
      discount_previous : 0,



///////////// add  item info  when we add item in invoice ///////

      item_add_form:false,
      items:[],  ///  this items used for add item when we add item in 
      payments:[],
      ...
      quentity_ref:"",     //// this  is used for campair value
...
/////////////// data for handling admin client mail///////////////
...
instantRetriveCampaign=()=>{
  this.setState({is_loader_open:true},()=>{
 fetch(`${base.base_url}/retriveCampaignForInvoice`,{
    headers:{
    ...
    this.setState({campaignList:res.data,is_loader_open:false})
...
retriveSingleClientInvoiceDetails=()=>{
 ...
 fetch(`${base.base_url}/retriveSingleInvoiceData`,{
  ...
  invoice_id:this.props.param.invoiceid.replace(/:/g,'')
    ...
    this.setState({
      invoice_sender_address:resultt.data.sender_address,singleInvoiceDetails:resultt.data,items:resultt.data.items,payments:resultt.data.payments,is_loader_open:false,discount:parseInt(resultt.data.discounts),has_discount:resultt.data.has_discount
    })
  })
  ...
  componentDidMount(){  ///// getSingleClientData
    this.setState({
      is_loader_open:true
    },()=>{
       fetch(`${base.base_url}/retriveSingleInvoiceData`,{
        ...
        this.setState({invoice_sender_address:resultt.data.sender_address,singleInvoiceDetails:resultt.data,items:resultt.data.items,payments:resultt.data.payments,discount:parseInt(resultt.data.discounts),has_discount:resultt.data.has_discount},()=>{
...
sendEmailInvoiceToClient=()=>{
  this.setState({open_loader_for_edit_page_popup:true,is_loader_open:true},()=>{
    ...
    fetch(`${base.base_url}/createInvoiceWithAttachments`,{
     headers:{
      'authorization' : `Bearer ${sessionStorage.getItem('token')}`,
       'content-type':'application/json',
     },
...
///////////////// payment done amount ////////////
      ...
      let finalPaymentRemains =  total_Amount - total_payment_done - this.state.discount ;
...
<Typography sx={{fontWeight:'700',fontSize:16,ml:0.5,mr:0.5}}>{this.props.param.invoiceid.replace(/:/g,'')}</Typography>
</Box>
</Box>
      ...
      <Table size="small" aria-label="a dense table">
        ...
        <TableHead sx={{backgroundColor:'#d3e2f8'}}>
          ...
          <TableCell align='left' sx={{fontSize:12,fontWeight:'600',color:'#4a3e40',borderRight:1,width:'5%'}}>NO</TableCell>
            ...
            <TableCell align='center' sx={{fontSize:12,fontWeight:'600',color:'#4a3e40',borderRight:1}}>CPL</TableCell>
        ...
        <TableBody>
          {this.state.items.map((row,i) => (
            ...
            >
              ...
              <TableCell align='left' sx={{color:'#42526e',fontWeight:'600',fontSize:12,width:'40%',borderRight:1,textTransform:'uppercase'}}>{row.campaignName}</TableCell>
              <TableCell align='center' sx={{color:'#42526e',fontWeight:'600',fontSize:12,borderRight:1}}>{(row.costPerLead).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</TableCell>
              ...
              <TableCell align='center' sx={{color:'#42526e',fontWeight:'600',fontSize:12,borderRight:1}}>{(row.quentity * row.costPerLead).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</TableCell>
...
<Box sx={{display:'flex',mt:2}}>
  <Checkbox size='small' checked={this.state.has_discount}  onChange={()=>this.state.has_discount? this.setState({has_discount:false,is_loader_open:true,discount_previous:this.state.discount},()=>{
    fetch(`${base.base_url}/updateDiscount`,{
      headers:{
        'authorization' : `Bearer ${sessionStorage.getItem('token')}`,
        'content-type':'application/json',
      },
     ...
     client_id:this.state.singleInvoiceDetails.client_id,
     invoice_id:this.props.param.invoiceid.replace(/:/g,''),
     discount_previous :this.state.discount,
     discount_ref : 0,
     has_discount:false,
      })
...
<Divider/>
    <Typography sx={{fontWeight:'600',mt:0.5,mb:1,color:'#4a3e40',fontSize:{xs:13,sm:14},display:this.state.has_discount?'flex':'none',flexDirection:'row',mt:2}}>Discount<Typography sx={{color:'#4a3e40',fontSize:{xs:13,sm:14},fontWeight:'600',ml:1}}>: $ {this.state.discount}<IconButton onClick={()=>this.setState({is_discount_add_form_open:true,discount_ref:parseInt(this.state.discount),discount_previous : parseInt(this.state.discount)})} sx={{backgroundColor:'#f2f8ff ',ml:1.5}} size='small'><DriveFileRenameOutlineIcon color='primary' sx={{height:17,width:17}}/></IconButton></Typography></Typography>
<Divider/>
...
<Box sx={{minHeight:60,backgroundColor:'#fff',padding:2,display:'flex',flexDirection:'column'}}>
    <Typography sx={{fontWeight:'600',color:'#4a3e40',mt:0.5,mb:0.5,fontSize:14,display:'flex',flexDirection:'row'}}>Client Name : <Typography sx={{color:'#259efa',fontSize:13,fontWeight:'600',ml:1}} > {this.state.sngleClientDetails?this.state.sngleClientDetails.client_name:""}</Typography></Typography>
...
(e)=>{
  const onlyNums = e.target.value.replace(/[^0-9]/g, '');
      if (onlyNums <= parseInt(finalPaymentRemains)) {
          this.setState({ payment_amount: onlyNums });
      } 
  }
...
if(this.state.payment_method!=="" && this.state.payment_amount!==0  && (this.state.payment_amount > 0) && this.state.payment_date!=""){
...
<TextField select SelectProps={{MenuProps:{sx:{height:300}}}} type='text' onChange={this.handleChange} InputProps={{sx:{fontSize:12,fontWeight:'600'}}} value={this.state.campain_name}  name="campain_name" fullWidth size='small'>
  {
    this.state.campaignList.map((w,index)=>(
      <MenuItem  key={index} value={w.campaign_name} sx={{fontSize:12,fontWeight:'600'}} onClick={()=>{this.setState({campaign_name:w.campaign_name, campain_id:w.campaign_id,  quentity: parseInt(w.lead_target) - w.no_of_leads_get_paid, quentity_ref:parseInt(w.lead_target) - w.no_of_leads_get_paid, costPerLead:w.cost_per_lead})}}>
      {
        w.campaign_name
      }
      </MenuItem>
...
const onlyNums = e.target.value.replace(/[^0-9]/g, '');
    if (onlyNums <= parseInt(this.state.quentity_ref)) {
        this.setState({ quentity: parseInt(onlyNums) });
    } 
}
...
<Button onClick={()=>{
if(this.state.campain_name!=="" && this.state.campain_id!=="" &&  parseInt(this.state.costPerLead) > 0 && parseInt(this.state.quentity) > 0 ){
...
>
  <Box sx={{display:'flex',justifyContent:'center',alignItems:'center',height:'100%',width:'100%'}}>
<Paper sx={{width:{xs:'90%',sm:'90%',md:'50%',lg:'35%'},height:'25vh',backgroundColor:'white',borderRadius:2}}>
<Box sx={{width:'100%',display:'flex',justifyContent:'right'}}>
<Paper onClick={()=>{this.setState({
is_discount_add_form_open:false
  })
...
discount_previous :this.state.discount_previous,
discount_ref  :this.state.discount_ref,
...
>
  <Box sx={{display:'flex',justifyContent:'center',alignItems:'center',height:'100%',width:'100%'}}>
<Paper sx={{width:{xs:'90%',sm:'90%',md:'50%',lg:'35%'},height:'80vh',backgroundColor:'white',borderRadius:2}}>
