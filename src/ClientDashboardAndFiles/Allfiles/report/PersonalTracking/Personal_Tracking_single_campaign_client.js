...
export class Personal_Tracking_single_campaign_client extends Component {
...
this.setState({is_loader_open_circle:true},()=>{
  ...
  fetch(`${base.base_url}/getSingleCampaignDetailsDataReportClient`,{
        headers:{
        ...
        method:'post',
        body:JSON.stringify({
            campaign_id : this.props.param.campaign_id.replace(/:/g,''),
        })
...
<Typography sx={{fontSize:12,fontWeight:'600',color:'#9a9a9a'}}>Uploaded<br/>Leads</Typography>
</Box>
...
<Chart
           options = {{
            series: [this.state.campaignData? (parseInt(this.state.campaignData.lead_target)  -  this.state.campaignData.total_upoaded_Leads)/parseInt(this.state.campaignData.lead_target) * 100 : 0],
            chart: {
            height: 350,
            type: 'radialBar',
          },
...
<Grid container  spacing={{xs:1,sm:2}}>
<Grid item xs={12} sm={4} md={4}>
<Typography sx={{mt:1,mb:1,ml:2,fontSize:14,fontWeight:'600'}}>Start Date : {moment(this.state.campaignData.start_date).format('MM-DD-YYYY')}</Typography>
<Typography sx={{ml:2,fontSize:14,fontWeight:'600'}}>End Date : {moment(this.state.campaignData.end_date).format('MM-DD-YYYY')}</Typography>
</Grid>
<Grid item xs={12} sm={4} md={4}>
...
export default Personal_Tracking_single_campaign_client
export function Personal_Tracking_single_campaign_clientc(props){
  const navigate = useNavigate();
  const location = useLocation();
  const param = useParams();
  return (<Personal_Tracking_single_campaign_client location={location} param={param} navigate={navigate}></Personal_Tracking_single_campaign_client>)
}
