...
import { Box, Paper, Tooltip, Backdrop, IconButton, CircularProgress, TextField, Button, Modal, Divider, TablePagination, Typography, TableBody, Table, TableContainer, TableCell, TableHead, TableRow, Checkbox, MenuItem, touchRippleClasses, Autocomplete, } from '@mui/material'
...
import { fillDepartment } from '../../../Utils/AutocompleteOptions';
...
export class SingleClientContact extends Component {
  ...
  contactExist = () => {
    toast.error(<Typography sx={{ fontSize: 13, fontWeight: 'bold' }}>Contact Exists</Typography>, {
      position: "top-right",
      autoClose: 4000,
      hideProgressBar: false,
      closeOnClick: true,
          ...
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row', md: 'row' }, justifyContent: 'space-between' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row', md: 'row' }, mr: 1, ml: 1 }}>
              <Box sx={{ backgroundColor: '#f8f9ff', borderRadius: 1, mt: { xs: 1, sm: 1, md: 1 }, height: 30, marginRight: { xs: 1, sm: 1, md: 0 } }}>
                <TextField type='text' fullWidth name='search' onChange={this.handleChange} variant='standard' InputProps={{ startAdornment: <SearchIcon sx={{ color: '#919191', mt: 0.5 }} />, disableUnderline: true, sx: { fontSize: "13px", fontWeight: '600', color: '#666666' } }} placeholder='Search' sx={{
                  "& input::placeholder": {
                    fontSize: "13px",
                ...
                <TableBody>
                  {mmmm.map((row, i) => (
                    ...
                    >
                      ...
                      <TableCell align='center' > <Box sx={{ height: 23, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Typography sx={{ paddingLeft: 0.4, paddingRight: 0.4, color: '#fff', fontWeight: '600', fontSize: 12, backgroundColor: '#2d85d1' }}> {row.department_name}</Typography></Box> </TableCell>
                      ...
                      <TableCell align='center' sx={{ color: '#42526e', fontSize: 13, textTransform: 'capitalize' }} onClick={async () => {
                        ...
                        <Box sx={{ height: 23, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Typography sx={{ paddingLeft: 0.4, paddingRight: 0.4, color: '#fff', fontWeight: '600', fontSize: 12, backgroundColor: '#2d85d1' }}><Link style={{ color: '#fff' }}>{row.first_name + " " + row.last_name}</Link></Typography></Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width: '100%' }}>
              <Paper sx={{ width: { xs: '90%', sm: '90%', md: '60%', lg: '30%' }, height: '90vh', backgroundColor: 'white', borderRadius: 2 }}>
                ...
                <Box sx={{ paddingLeft: { xs: 2, sm: 4 }, paddingRight: { xs: 2, sm: 4 }, overflowY: 'scroll', maxHeight: '75vh' }}>
                  ...
                  {/* <TextField SelectProps={{MenuProps:{sx:{height:300}}}} select InputProps={{sx:{fontSize:12,fontWeight:'600',textTransform:'capitalize'}}} onChange={this.handleChange}  value={this.state.department_name} name="department_name" fullWidth size='small'>
 {this.state.departmentList.map((dept_data)=>(
  <MenuItem sx={{fontSize:12,fontWeight:'600'}} key={dept_data.department_id} 
  onClick={()=>this.setState({department_id:dept_data.department_id,department_name:dept_data.department_name})}
   value={dept_data.department_name}>{dept_data.department_name}</MenuItem>
 ))
 } 
</TextField> */}
                  ...
                  <Typography sx={{ fontSize: 12, mt: 1, fontWeight: '600', padding: 0.2, display: 'flex', flexDirection: 'row' }}>First Name <Typography sx={{ color: 'red' }}>*</Typography></Typography>
                          ...
                          <InputAdornment position="start">
                            {
                              this.state.showpassword ?
                                <Visibility style={{ color: '#a2a2a6' }} onClick={() => this.setState({ showpassword: false })} /> :
                                <VisibilityOff onClick={() => this.setState({ showpassword: true })} style={{ color: '#a2a2a6' }} />
                            }
                          </InputAdornment>
                          <Tooltip title="Reset Password">
                            <IconButton onClick={() => this.setState({ password: password.randomPassword({ characters: password.lower + password.upper + password.digits }) })}>
                              <SyncLockIcon sx={{ color: '#2486bb' }} />
                            </IconButton>
                          </Tooltip>
                        </>,
              ...
              <Paper sx={{ width: { xs: '90%', sm: '90%', md: '60%', lg: '30%' }, height: '90vh', backgroundColor: 'white', borderRadius: 2 }}>
                ...
                <Box sx={{ paddingLeft: { xs: 2, sm: 4 }, paddingRight: { xs: 2, sm: 4 }, overflowY: 'scroll', maxHeight: '75vh' }}>
                  ...
                  <Autocomplete
                    ...
                    size="small"
                    defaultValue={{ id: this.state?.department_id_search, label: this.state.department_name }}
                  ...
                  <Typography sx={{ fontSize: 12, fontWeight: '600', padding: 0.2, mt: 1, display: 'flex', flexDirection: 'row' }}>First Name <Typography sx={{ color: 'red' }}>*</Typography></Typography>
                  ...
                  <TextField type={this.state.showpassword ? 'text' : 'password'} name="password" onChange={this.handleChange} value={this.state.password}
                    ...
                    //         <SyncLockIcon sx={{ color: '#2486bb' }} />
                    //       </IconButton>
                    //     </Tooltip>, sx: { fontSize: 12, fontWeight: '600' }
                    // }}
