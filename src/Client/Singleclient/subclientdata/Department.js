...
<Box>
<Box sx={{minHeight:50,width:'100%',backgroundColor:"#fff",display:'flex',flexDirection:{xs:'column',sm:'row',md:'row'},justifyContent:'space-between',}}>
...
<Box>
 <Button disabled={this.state.client_role.is_create?false:true} onClick={()=>this.setState({open_form_add_dept:true})}  sx={{textTransform:'none',mt:{xs:1,sm:1,md:0},height:30,backgroundColor:'#008ffb',fontWeight:'600',minWidth:100}} disableElevation variant="contained" startIcon={<AddIcon sx={{color:'#fff'}}/>}>
Add Department
</Button> 
</Box>
</Box>
        ...
        <TableBody>
          {mm.map((row,index) => (
            ...
            this.setState({contact_List:result.data,is_backdrop_open:false})
              ...
              <Box sx={{height:23,display:'flex',justifyContent:'center',alignItems:'center'}}><Typography sx={{paddingLeft:0.4,paddingRight:0.4,color:'#fff',fontWeight:'600',fontSize:12,backgroundColor:'#2d85d1'}}> <Link style={{textDecoration:'underline',color:'#fff'}}>{row.department_name}</Link></Typography></Box> 
...
<Tooltip title="Edit">
  <IconButton disabled={this.state.client_role.is_edit?false:true} size='small' onClick={()=>this.setState({
  edit_dept_form:true,
