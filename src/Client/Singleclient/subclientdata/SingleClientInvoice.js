...
import {Box,Paper,Tooltip,IconButton,Backdrop,CircularProgress,TextField,Button,ButtonGroup ,MenuItem,TablePagination,Modal,Typography,TableBody,Link,Table,TableContainer,TableCell,TableHead,TableRow,Divider} from '@mui/material'
...
import { SingleClientServiceInvoicec } from './SingleClientServiceInvoice';
...
>
  <Button size='small' variant={this.state.is_invoice=="invoice"?'contained':'outlined'} onClick={()=>this.setState({is_invoice:'invoice'})} sx={{textTransform:'none',fontWeight:'600',backgroundColor:this.state.is_invoice=='invoice'?'#008ff':'#fff'}} startIcon={<PaidIcon/>}>Camapign Invoice</Button>
<Button size='small' variant={this.state.is_invoice=="services"?'contained':'outlined'} onClick={()=>this.setState({is_invoice:'services'})} sx={{textTransform:'none',fontWeight:'600',backgroundColor:this.state.is_invoice=='services'?'#008ff':'#fff'}} startIcon={<AttachMoneyIcon/>}>Services Invoice</Button>
        ...
        <TableBody>
          {this.state.invoiceList.map((row,index) => (
            ...
            >
              ...
              <TableCell align='center' sx={{color:'#42526e',fontWeight:'600',fontSize:13}}><CountUp start={0} end={row.items.reduce( ( sum , cur ) => sum + parseInt(cur.quentity) * parseInt(cur.costPerLead) , 0) - parseInt(row.discounts)   }  /></TableCell>
              ...
              <TableCell align='center' sx={{color:'#42526e',fontWeight:'600',fontSize:13}}><CountUp start={0} end={row.items.reduce( ( sum , cur ) => sum + parseInt(cur.quentity) * parseInt(cur.costPerLead) , 0)  - row.payments.reduce( ( sum , cur ) => sum + parseInt(cur.payment_amount) , 0) - parseInt(row.discounts)  }/></TableCell>
...
this.setState({
  ...
  total_invoice:row.items.reduce( ( sum , cur ) => sum + parseInt(cur.quentity) * parseInt(cur.costPerLead) , 0)  ,
