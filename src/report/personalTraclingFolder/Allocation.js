...
monthly_allocation.push({
  name:this.props.monthly_allocation[j].client_name,
  type:'column',
...
for (let w = 0; w <  this.props.yearly_revenue_and_allocation.length; w++) {
  ...
  let imagenary_arary = []
       ...
       for (let l = 0; l < this.props.year_list.length?this.props.year_list.length:0; l++) {
        ...
        name:this.props.yearly_revenue_and_allocation[w][0].client_name,
        type:'column',
...
for (let ss = 0; ss < this.props.quaterly_allocation.length; ss++) {
  quaterly_allocation.push({
    name:this.props.quaterly_allocation[ss].client_name,
    type:'column',
    ...
    return (
      ...
      <Paper sx={{minHeight:0,width:'100%',display:this.props.data?this.props.data.client_total_allocation_and_revenue?"block":"none":'none'}}>
       <Typography sx={{fontSize:15,fontWeight:'bold',padding:1,pl:2,color:'#717171'}}>Allocation Till Date</Typography>
       <Divider/>
       <Box>
          

{this.props.data?this.props.data.client_total_allocation_and_revenue?this.props.data.client_total_allocation_and_revenue.map((s)=>(
